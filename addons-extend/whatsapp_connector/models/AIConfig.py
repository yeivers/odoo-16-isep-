...
class AIConfig(models.Model):
    ...
    def execute_ai(self, prompt=None, conversation=None):
        self.ensure_one()
        if not conversation and not prompt:
            raise ValidationError(_('You must provide a conversation or prompt.'))
        out = None
        if self.provider == 'openai':
            out = self.execute_openai(prompt, conversation)
        return out

    def execute_openai(self, prompt, conversation):
        self.ensure_one()
        out = None
        usage_log = self.create_usage_log(conversation)
        if self.operation_key == 'chat_completions':
            if not conversation:
                raise ValidationError(_('Conversation is required.'))
    ...
    def make_request(self, data_to_process, usage_log):
        self.ensure_one()
        url = self.get_url()
        headers = self.sudo().get_header()
        data = self.get_body(data_to_process)
        _logger.info(f'\n  post => {url}')
        ...
        if 200 <= req.status_code < 300:
            out = self.handle_response(req, usage_log)
        else:
            self.handle_request_error(req)
        return out

    def handle_response(self, req: requests.Response, usage_log):
        self.ensure_one()
        data = req.json()
        out = None
        if self.provider == 'openai':
    ...
    def handle_request_error(self, req: requests.Response):
        ...
        try:
            ...
            raise ValidationError(req.text if req.text else _('An error occurred.'))
        if error is not None:
            self.handle_json_error(error)

    def handle_json_error(self, error: dict):
        self.ensure_one()
        if self.provider == 'openai':
            err = error.get('error', {})
            err = err.get('message') or err.get('code', '').replace('_', ' ').upper()
            raise ValidationError(err or _('An error occurred.'))

    def get_url(self):
        self.ensure_one()
        out = ''
        if self.provider == 'openai':
            out = f'https://api.openai.com/v1/{self.operation_key.replace("_", "/")}'
        return out
    ...
    def get_body(self, data_to_process):
        '''
        ...
        if self.provider == 'openai':
            ...
            if self.operation_key == 'completions':
                out.update({
                    ...
                    'prompt': f'{self.command} {data_to_process}'
                })
                if self.add_roles:
