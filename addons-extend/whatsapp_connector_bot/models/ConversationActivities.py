...
class ConversationActivities(models.Model):
    ...
    ttype = fields.Selection(selection_add=[('bot_thread', 'BOT thread'),
                                            ('bot_mute', 'BOT mute'),
                                            ('bot_mute_rem', 'BOT reminder mute')],
                             ondelete={'bot_thread': 'cascade', 'bot_mute': 'cascade', 'bot_mute_rem': 'cascade'})

    @api.autovacuum
    def _gc_chat_bot(self):
        self.clean_chat_bot(max_thread_hours=96, max_mute_hours=96)
