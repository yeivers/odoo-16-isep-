var PdfFlip = {
    ...
    init: function () {
        ...
        $(document).on('click','.outlineItem a',function(){
          $('.toolbar .pageNumber').trigger('change');
        });

        $(document).on('click','#ExitPresentationMode',function(event){
        ...
        $(document).on('click','#BackToLibrary',function(event){
            top.window.location = '/my-library';
        });

        $(document).on('change', '.toolbar .pageNumber', function (e) {
            $("#magazine").turn('page', $(this).val());
//            var page_src = '/material/set_last_read_page/#page=' + $(this).val();
//            $('#pdf_render_i_frame').attr('page',page_src);
        });
    ...
    start: function () {
        ...
        PDFViewerApplication.initialBookmark = "page=10";
    ...
    loadTurnJsPages: function (pages, magazine, isInit, defer, scale) {
        ...
        for (var i = 0; i < pages.length; i++) {
            PDFViewerApplication.pdfDocument.getPage(pages[i]).then(function (page) {
                ...
                //destinationCanvas.style.height = "100%";
