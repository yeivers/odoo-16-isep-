...
class HrPayslip(models.Model):
    ...
    def compute_sheet(self):
        for rec in self:
            ...
            for earn_id in rec.earn_ids:
                earn_list.append({
                    'input_type_id': earn_id.rule_input_id.id,
            ...
            # rec.update({'worked_days_line_ids': worked_days_line_list}) #Validar por que pasar valores worked_days si se pasan en imputs
    ...
    def compute_totals(self):
        for rec in self:
            # Validate if it is a payroll for Colombia:
            if rec.company_id.edi_payroll_enable or rec.company_id.edi_payroll_consolidated_enable:
    ...
    def get_json_request(self):
        for rec in self:
            ...
            for earn_id in rec.earn_ids:
                if not earn_id.rule_input_id.rule_id.edi_is_detailed:
                    raise UserError(_("This concept must be calculated through the salary rules: %s")
                                    % earn_id.rule_input_id.rule_id.name)
