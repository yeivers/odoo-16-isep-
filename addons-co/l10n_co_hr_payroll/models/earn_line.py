...
class EarnLine(models.Model):
    ...
    rule_input_id = fields.Many2one('hr.payslip.input.type', string='Rule input', copy=True, required=True,
                                    domain=[('rule_id.type_concept', '=', 'earn')])
    ...
    def _compute_rule(self):
        for rec in self:
            ...
            rec.category = rec.rule_input_id.rule_id.earn_category #Se adapta para ingregrase con enterprice
